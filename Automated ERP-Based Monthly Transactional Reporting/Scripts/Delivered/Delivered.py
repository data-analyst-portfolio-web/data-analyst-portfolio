...
# Folder containing your "Delivered" Excel files
folder_path = r"C:\Users\CCE\Documents\Cool Creek Docs\Reports\Melinda\Raw\Delivered"
output_folder = r"C:\Users\CCE\Documents\Cool Creek Docs\Reports\Melinda\Raw\Delivered"
merged_output_file_base = r"C:\Users\CCE\Documents\Cool Creek Docs\Reports\Melinda\Raw\Delivered\Merged_Delivered.xlsx"

# Define GAS/Diesel formula logic
def gas_diesel_formula(value):
    value = str(value).upper()
    if any(keyword in value for keyword in ["GASOLINE", "GAS", "ETHANOL", "BLEND"]):
        return "GAS"
    else:
        return "Diesel"
...
def assigned_unassigned_formula(value):
    value = str(value).upper()
    if any(keyword in value for keyword in ["IOLF", "IOL"]):
        return "Assigned"
    else:
        return "Unassigned"
...
delivered_files = [file for file in os.listdir(folder_path) if file.lower().startswith("delivered") and file.lower().endswith('.xlsx')]


# Check if there are no matching files
if not delivered_files:
    print("No 'Delivered' files found in the folder.")
    sys.exit()  # Exit the program
else:
    # Process each "Delivered" file
    for file_name in os.listdir(folder_path):
        if file_name.lower().startswith("delivered") and file_name.lower().endswith('.xlsx'):
            ...
            # Delete rows based on conditions (case-insensitive)
            df = df[~df["ADDRESS1"].str.upper().str.contains("CARDLOCK", na=False)]
            df = df[~df["ADDRESS1"].str.upper().str.contains("C/L", na=False)]
            df = df[~df["NAME"].str.upper().str.contains("COOL CREEK", na=False)]
            df = df[~df["NAME"].str.upper().str.contains("ROCKY MOUNTAIN ENERGY", na=False)]
            df = df[~df["NAME"].str.upper().str.contains("CARDLOCK", na=False)]
            ...
            df["Assigned/Unassigned"] = df["CUSTOMER_ID"].apply(assigned_unassigned_formula)
            
            # Add "GAS/Diesel" column
            df["GAS/Diesel"] = df["PROD_DESC"].apply(gas_diesel_formula)
            
            # Add "Mon/Year" column
            df["YearMon"] = df["SHIP_DATE"].apply(yearmon_formula)
...
# Ensure SHIP_DATE is in datetime format
merged_df["SHIP_DATE"] = pd.to_datetime(merged_df["SHIP_DATE"], errors="coerce")

# Group data by year
merged_df["Year"] = merged_df["SHIP_DATE"].dt.year
...
for year, year_df in merged_df.groupby("Year"):
    ...
    if pd.notna(year):  # Skip rows with invalid dates
        ...
        if os.path.exists(output_file):
            ...
            combined_df = pd.concat([existing_df, year_df], ignore_index=True).drop_duplicates(subset=existing_df.columns[:27])

        else:
            # Directly use the new data if the file does not exist
            combined_df = year_df.drop_duplicates(subset=year_df.columns[:27])
