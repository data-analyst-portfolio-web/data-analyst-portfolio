...
# Folder containing your "External" Excel files
...
merged_output_file = r"C:\Users\CCE\Documents\Cool Creek Docs\Reports\Melinda\Raw\CL\Merged_External"

# Columns to delete
columns_to_delete = [
    "SORT_BY", "CNOTE", "SEQUENCE", 
    "NDISCNTAMT", "NCLFEE", "NCOSTLESSFEE", "NMARGIN", "NODOMETER"
]
...
def assignment_formula(database, cclsite):
    database = str(database).upper()
    cclsite = str(cclsite).upper()
    
    rocky_locations = [
        "MERRITT", "KAMLOOP", "LILLOOET", "CLEARWATER", "PRINCETON", "LUMBY",
        "BARRIERE", "ARMSTRONG", "OLIVER", "OSOYOOS", "ASHCROFT", "KELOWNA",
        "GOLD BRIDGE", "FAUQUIER", "HALSTON", "CHILLIWACK", "ENDERBY", "BLUE", "AGASSIZ"
    ]
    cool_creek_locations = [
        "CASTLEGAR", "SALMO", "CRANBROOK", "MIDWAY", "SPARWOOD"
    ]
    
    if database in ["CASTLEGAR", "CRANBROOK"] and any(loc in cclsite for loc in rocky_locations):
        return "ROCKY CUST TO COOL CREEK CLSITE"
    elif database in ["CCE", "HINTON"] and any(loc in cclsite for loc in cool_creek_locations):
        return "COOL CREEK CUST TO ROCKY CLSITE"
    else:
        return "OTHER"
...
external_files = [file for file in os.listdir(folder_path) if file.lower().startswith("external") and file.lower().endswith('.xlsx')]

# Check if there are no matching files
if not external_files:
    print("No 'External' files found in the folder.")
    sys.exit()  # Exit the program
else:
    # Process each "External" file
    for file_name in os.listdir(folder_path):
        if file_name.lower().startswith("external") and file_name.lower().endswith('.xlsx'):
            ...
            elif "hinton" in file_name_lower:
                database_value = "Hinton"
            else:
            ...
            df["Assigned/Unassigned"] = "External"
            
            # Add "GAS/Diesel" column
            df["GAS/Diesel"] = df["CPRODDS"].apply(gas_diesel_formula)
            
            # Add "YearMon" column
            df["YearMon"] = df["INVOICE_DATE"].apply(yearmon_formula)
            
            # Add "Clear/Dyed" column
            df["Clear/Dyed"] = df["CPRODDS"].apply(clear_dyed_formula)
            
            # Add "PROFIT" column (after AMOUNT column)
            if "PROFIT" not in df.columns:
                if "AMOUNT" in df.columns and "COST_AMOUNT" in df.columns:
                    df.insert(df.columns.get_loc("AMOUNT") + 1, "PROFIT", df["AMOUNT"] - df["COST_AMOUNT"])
                else:
                    print(f"Missing 'AMOUNT' or 'COST_AMOUNT' column in file: {file_name}")
            else:
                print(f"'PROFIT' column already exists in file: {file_name}")
                
            
            # Add "ASSIGNMENT" column (after "YearMon" column)
            if "Database" in df.columns and "CCLSITE" in df.columns:
                df.insert(
                    df.columns.get_loc("YearMon") + 1, 
                    "ASSIGNMENT", 
                    df.apply(lambda x: assignment_formula(x["Database"], x["CCLSITE"]), axis=1)
                )
            else:
                print(f"Missing 'Database' or 'CCLSITE' column in file: {file_name}")
...
for year, year_df in merged_df.groupby("Year"):
    ...
    if pd.notna(year):  # Skip rows with invalid dates
        output_file = f"{merged_output_file}_{int(year)}.xlsx"
        ...
        if os.path.exists(output_file):
            ...
            combined_df = pd.concat([existing_df, year_df], ignore_index=True).drop_duplicates(subset=existing_df.columns[:21])

        else:
            # Directly use the new data if the file does not exist
            combined_df = year_df.drop_duplicates(subset=year_df.columns[:21])
