...
output_file = r"C:\Users\CCE\Documents\Cool Creek Docs\Reports\Melinda\Raw\CL\Summary\CL_External_summ_cust.csv"

# List all Excel files that start with "Merged_External"
excel_files = [f for f in os.listdir(folder_path) if f.startswith("Merged_External") and f.endswith((".xlsx", ".xls"))]

# Initialize an empty list to store DataFrames
data_frames = []

# Process each "Merged_External" Excel file
for file_name in excel_files:
    ...
    try:
        ...
        df = pd.read_excel(file_path, dtype={"CUSTOMER_ID": str}, usecols=[
            ...
            "Assigned/Unassigned", "YearMon", "Clear/Dyed","ASSIGNMENT"
        ])
...
if data_frames:
    ...
    result = combined_df.groupby(["CUSTOMER_ID", "CUSTOMER_NAME", "PRODUCT_ID", "CPRODDS", "CCLSITE", "Database", 
                                  "MOD", "Assigned/Unassigned", "YearMon", "Clear/Dyed","ASSIGNMENT"], as_index=False).agg({
    ...
    transaction_counts = combined_df.groupby(["CUSTOMER_ID", "CUSTOMER_NAME", "PRODUCT_ID", "CPRODDS", "CCLSITE", "Database", 
                                              "MOD", "Assigned/Unassigned", "YearMon", "Clear/Dyed","ASSIGNMENT"]).size().reset_index(name="TRANSACTION_COUNT")
    ...
    result = result.merge(transaction_counts, on=["CUSTOMER_ID", "CUSTOMER_NAME", "PRODUCT_ID", "CPRODDS", "CCLSITE", "Database", 
                                                  "MOD", "Assigned/Unassigned", "YearMon", "Clear/Dyed","ASSIGNMENT"])
    ...
    column_order = ["CUSTOMER_ID", "TRANSACTION_COUNT", "CUSTOMER_NAME", "PRODUCT_ID", "CPRODDS", "CCLSITE", "Database", 
                    "MOD", "Assigned/Unassigned", "YearMon", "Clear/Dyed","ASSIGNMENT", "VOLUME", "COST_AMOUNT", "AMOUNT", "PROFIT"]
    ...
    print("No 'Merged_External' Excel files found in the folder.")
